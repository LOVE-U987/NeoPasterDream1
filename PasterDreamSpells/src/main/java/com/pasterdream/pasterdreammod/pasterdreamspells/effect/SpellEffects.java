package com.pasterdream.pasterdreammod.pasterdreamspells.effect;
...
public final class SpellEffects {
    ...
    public static void lightning(ServerLevel level, double x, double y, double z) {
        ...
        schedule(level, 2, () -> {
            playSound(level, x, y, z, PDSpellsSounds.LIGHTNING_SPELL.get(), 1.0f);
    ...
    public static void poison(ServerLevel level, double x, double y, double z) {
        playSound(level, x, y, z, SoundEvents.SPLASH_POTION_BREAK, 1.0f);
        level.sendParticles(PDSpellsParticles.POISON_GAS_PARTICLE.holder().get(), x, y + 1, z, 48, 2, 1, 2, 0.01);
        level.sendParticles(PDSpellsParticles.POISON_SOUL_PARTICLE.holder().get(), x, y + 1, z, 24, 2, 1, 2, 0.02);

        for (int base : POISON_WAVE_BASE_TICKS) {
            // 效果脉冲：粒子 + 施加剧毒/虚弱/缓慢
            for (int pulse : POISON_EFFECT_PULSE_TICKS) {
                schedule(level, base + pulse, () -> {
                    level.sendParticles(PDSpellsParticles.POISON_GAS_PARTICLE.holder().get(),
                            x, y + 1, z, 64, 2, 1, 2, 0.01);
                    level.sendParticles(PDSpellsParticles.POISON_SOUL_PARTICLE.holder().get(),
    ...
    public static void healing(ServerLevel level, double x, double y, double z) {
        playSound(level, x, y, z, SoundEvents.SPLASH_POTION_BREAK, 1.0f);
        level.sendParticles(PDSpellsParticles.HEALING_SPELL_PARTICLE.holder().get(), x, y + 1, z, 64, 2, 1, 2, 0.05);
        level.sendParticles(PDSpellsParticles.YELLOW_SMOKE_PARTICLE.holder().get(), x, y + 0.5, z, 32, 2, 1, 2, 0.05);
        spawnField(level, PDSpellsEntities.HEALING_SPELL_ENTITY.get(), x, y, z);
        schedule(level, 2, () -> playSound(level, x, y, z, PDSpellsSounds.HEALING_SPELL.get(), 0.2f));
    }
    ...
    public static void fury(ServerLevel level, double x, double y, double z) {
        ...
        playSound(level, x, y, z, PDSpellsSounds.FURY_SPELL_0.get(), 1.0f);
        spawnField(level, PDSpellsEntities.FURY_SPELL_ENTITY.get(), x, y, z);
    ...
    private static void furyPulse(ServerLevel level, double x, double y, double z) {
        ...
        level.sendParticles(PDSpellsParticles.FURY_SPELL_PARTICLE.holder().get(), x, y + 2, z, 12, 2.5, 1, 2.5, 0.02);
        ...
        for (Player player : players) {
            player.addEffect(new MobEffectInstance(PDSpellsEffects.FURY_SPELL_BUFF, 60, 0));
        }
    }
    ...
    public static void ice(ServerLevel level, double x, double y, double z) {
        playSound(level, x, y, z, SoundEvents.SPLASH_POTION_BREAK, 1.2f);
        playSound(level, x, y, z, PDSpellsSounds.ICE_SPELL.get(), 1.0f);
        level.sendParticles(ParticleTypes.SNOWFLAKE, x + 0.5, y + 1.5, z + 0.5, 240, 2.5, 1.5, 2.5, 0.05);
        level.sendParticles(PDSpellsParticles.SPELL_SNOWFLAKE_0_PARTICLE.holder().get(),
                x + 0.5, y + 2, z + 0.5, 128, 2.5, 1.5, 2.5, 0.1);
        level.sendParticles(PDSpellsParticles.SNOWFLAKE_1_PARTICLE.holder().get(),
                x + 0.5, y + 2, z + 0.5, 128, 2.5, 1.5, 2.5, 0.1);
    ...
    private static void iceFreezeWave(ServerLevel level, double x, double y, double z) {
        ...
        for (Entity target : targets) {
            ...
            if (target instanceof LivingEntity living) {
                living.addEffect(new MobEffectInstance(PDSpellsEffects.ICE_SPELL_BUFF, 40, 0));
            }
        }
    }
