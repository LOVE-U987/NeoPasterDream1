...
public class DyedreamWorldTreeTrunkPlacer extends TrunkPlacer {
    ...
    public List<FoliagePlacer.FoliageAttachment> placeTrunk(LevelSimulatedReader level, BiConsumer<BlockPos, BlockState> blockSetter,
        ...
        // 地面支撑检测：扫描 5x5 主柱 + 角柱（偏移3）覆盖范围内的最低地面 Y，
        // 若最低地面低于树根点，向下延伸主柱填补悬空
        int footprintRadius = 4; // 主柱±2 + 角柱偏移3 + cwOffset±1
     ...
     * <p>
     * 扫描范围覆盖5x5主柱（中心 ±2）与角柱（偏移3 + cwOffset ±1），即中心 ± {@code radius}。
     ...
     * <p>
     * 仅填充5x5主柱范围（中心 ±2，削角同主循环），角柱由各自的放置逻辑独立处理。
     * 仅替换空气方块，不破坏已有地形方块。
     *
     * @param level      模拟世界读取器
