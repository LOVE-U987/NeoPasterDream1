 ...
 * <ul>
 *   <li>锚点：屏幕右下（宽 + san tank xbase(-36)，高 + san tank ybase(-34)）</li>
 ...
 * </ul>
 * 显示条件：未骑乘生物、未隐藏 GUI、理智系统开启（客户端镜像开关）、
 * 且（若开启潜行显示配置）处于潜行。
 */
public class SanTankOverlay implements LayeredDraw.Layer {
    ...
    public void render(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        int xBase = width + PDClientConfig.SAN_TANK_XBASE.get().intValue();
        int yBase = height + PDClientConfig.SAN_TANK_YBASE.get().intValue();
        ...
        guiGraphics.blit(ICON, xBase, yBase + 6, 0, 70, 32, 20 - Math.round(amount * smallAmount));
        ...
        if (instance != null) {
            ...
            if (instance.getValue() > 0) {
                guiGraphics.blit(ICON, xBase + 18 + (small ? 3 : 0), yBase + 16 + (small ? 4 : 0),
        ...
        if (player.isShiftKeyDown()) {
            guiGraphics.drawString(mc.font, amount + "/100", xBase + 4, yBase - 4, -1);
        }
