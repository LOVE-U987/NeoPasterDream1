...
public class PDCommonConfig {
    ...
    static {
        ...
        MELTDREAM_CHEST_CUSTOM_LOOT_ENABLED = builder
                .comment("融梦水晶箱自定义战利品总开关（开启后按下方三个品质物品池生成掉落，逐条容错，全无效则用内置兜底池；关闭时使用原版战利品表） 默认：false")
        ...
        MELTDREAM_CHEST_COMMON_LOOT = builder
                .comment("融梦水晶箱普通品质物品池（每行一个条目，格式：物品ID 数量 权重，如 \"pasterdream:dyedream_ingot 1 10\"） 默认：空（留空或全无效时用内置兜底池）")
                .define("meltdream chest common loot", List.of(),
                        o -> o instanceof List);
        MELTDREAM_CHEST_RARE_LOOT = builder
                .comment("融梦水晶箱稀有品质物品池（格式同上） 默认：空")
                .define("meltdream chest rare loot", List.of(),
                        o -> o instanceof List);
        MELTDREAM_CHEST_LEGENDARY_LOOT = builder
                .comment("融梦水晶箱传说品质物品池（格式同上） 默认：空")
                .define("meltdream chest legendary loot", List.of(),
                        o -> o instanceof List);
        builder.pop();
