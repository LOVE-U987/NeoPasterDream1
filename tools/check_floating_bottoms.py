# -*- coding: utf-8 -*-
"""检查 dream_train / dyedream_floating_temple / dyedream_worldtree / dyedream_tower 底部设计"""
import gzip
import io
import struct
...
NAMES = ["dream_train", "dyedream_floating_temple", "dyedream_worldtree",
         "dyedream_worldtree_true", "dyedream_tower_0", "dyedream_tower_1"]
for name in NAMES:
    p = BASE / (name + ".nbt")
    if not p.exists():
    ...
    for layer in range(min(8, size[1])):
        cnt = Counter()
        for b in blocks:
            if b["pos"][1] == layer:
