...
| C2-4 | 满套 `addEffect` 无条件重加弱效果覆盖外来更强效果（已核验：vanilla `MobEffectInstance.update` 仅在「新效果更高等级/同级更长」时替换，10t/0级 不占优，外来更强效果不降级；仅 cosmetic 图标/粒子隐藏，维持现状不修） | `item/SculkArmorItem.java:43` `TitaniumArmorItem.java:43` `DyedreamArmorItem.java:43` | 4 | 已核验（不修）                       |
