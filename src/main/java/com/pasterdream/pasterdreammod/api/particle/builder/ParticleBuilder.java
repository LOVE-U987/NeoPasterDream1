...
public class ParticleBuilder {
    ...
    public ParticleBuilder(String modId, String name) {
        ...
        PasterDreamMod.LOGGER.debug("[ParticleBuilder] 创建粒子构建器: modId={}, name={}, 默认纹理={}", modId, name, texturePath);
    }
    ...
    public static ParticleBuilder builder(String name) {
        return new ParticleBuilder(PasterDreamMod.MOD_ID, name);
    }
    ...
    public ParticleBuilder alwaysShow() {
        ...
        PasterDreamMod.LOGGER.debug("[ParticleBuilder] {} → alwaysShow=true", name);
        return this;
    }
    ...
    public ParticleBuilder alwaysShow(boolean alwaysShow) {
        ...
        PasterDreamMod.LOGGER.debug("[ParticleBuilder] {} → alwaysShow={}", name, alwaysShow);
        return this;
    }
    ...
    public ParticleBuilder texture(String texturePath) {
        ...
        PasterDreamMod.LOGGER.debug("[ParticleBuilder] {} → texture={}", name, texturePath);
        return this;
    }
    ...
    public ParticleBuilder withGravity(float gravity) {
        ...
        PasterDreamMod.LOGGER.debug("[ParticleBuilder] {} → gravity={}", name, gravity);
        return this;
    }
    ...
    public ParticleBuilder generateJson(boolean generate) {
        ...
        PasterDreamMod.LOGGER.debug("[ParticleBuilder] {} → generateJson={}", name, generate);
        return this;
    }
    ...
    public ParticleBuilder basePath(String basePath) {
        ...
        PasterDreamMod.LOGGER.debug("[ParticleBuilder] {} → basePath={}", name, basePath);
        return this;
    }
    ...
    public ParticleResult build() {
        PasterDreamMod.LOGGER.info("[ParticleBuilder] ===== 开始构建粒子: {} =====", name);
        PasterDreamMod.LOGGER.info("[ParticleBuilder]   配置: alwaysShow={}, texture={}, gravity={}, generateJson={}",
                alwaysShow, texturePath, gravity, generateJsonFiles);

        // 注册 SimpleParticleType
        PasterDreamMod.LOGGER.debug("[ParticleBuilder] 注册 SimpleParticleType: {} (alwaysShow={})", name, alwaysShow);
        ...
        PasterDreamMod.LOGGER.info("[ParticleBuilder] ✅ SimpleParticleType 已注册: {} | holder={}", name, holder);

        // 生成资源文件
        if (generateJsonFiles) {
            try {
                PasterDreamMod.LOGGER.info("[ParticleBuilder] ===== 开始生成粒子资源文件: {} =====", name);

                // 生成 particles/{name}.json（粒子定义）
                PasterDreamMod.LOGGER.debug("[ParticleBuilder] 生成粒子定义 JSON: {}/{}.json", basePath, name);
                new ParticleGenerator(modId, name)
                        .addTexture(texturePath)
                        .saveToFile(basePath);

                // 生成 textures/particle/{name}.json（纹理元数据）
                PasterDreamMod.LOGGER.debug("[ParticleBuilder] 生成粒子纹理元数据: {} (gravity={})", name, gravity);
                new ParticleTextureGenerator(modId, name)
                        .withGravity(gravity)
                        .saveToFile(basePath);

                PasterDreamMod.LOGGER.info("[ParticleBuilder] ✅ 粒子资源文件生成完成: {}", name);
            } catch (IOException e) {
                PasterDreamMod.LOGGER.error("[ParticleBuilder] ❌ 无法生成粒子资源文件 [{}]: {}", name, e.getMessage(), e);
                throw new RuntimeException("ParticleBuilder: 无法生成粒子资源文件 [" + name + "]", e);
            }
        } else {
            PasterDreamMod.LOGGER.info("[ParticleBuilder] ⏭️ 跳过 JSON 文件生成: {} (generateJson=false)", name);
        }

        ParticleResult result = new ParticleResult(name, holder);
        PasterDreamMod.LOGGER.debug("[ParticleBuilder] 创建 ParticleResult: name={}, holder={}", name, holder);

        // 缓存到 ParticleAPI 中，方便后续查询
        ParticleAPI.cacheParticle(result);

        PasterDreamMod.LOGGER.info("[ParticleBuilder] ✅ 粒子构建完成: {} | result={}", name, result);
        return result;
    }
}