...
public final class RuinAPI {
    ...
    public static RuinBuilder createRuin(String name) {
        PasterDreamMod.LOGGER.info("[RuinAPI] 🏛️ 开始创建遗迹构建器: {}", name);
        return new RuinBuilder(PasterDreamMod.MOD_ID, name, REGISTRY);
    }
    ...
    public static StructureSetBuilder createRuinSet(String ruinName, String setName) {
        PasterDreamMod.LOGGER.info("[RuinAPI] 🔗 开始创建结构集构建器: ruin={}, setName={}", ruinName, setName);
        RuinResult result = getRuin(ruinName);
        if (result == null) {
            PasterDreamMod.LOGGER.error("[RuinAPI] ❌ createRuinSet 失败: 找不到结构 [{}]", ruinName);
            throw new IllegalStateException(
                    "RuinAPI.createRuinSet: 找不到结构 [" + ruinName + "]，请先调用 createRuin().build()"
            );
        }
        PasterDreamMod.LOGGER.debug("[RuinAPI] 找到关联结构 [{}]: structureKey={}", ruinName, result.structureKey());
        return new StructureSetBuilder(PasterDreamMod.MOD_ID, ruinName, setName);
    }
    ...
    public static RuinResult getRuin(String name) {
        ...
        PasterDreamMod.LOGGER.debug("[RuinAPI] 🔍 查询遗迹: {} → {}", name, result != null ? "已找到" : "未找到");
        return result;
    }
    ...
    public static Map<String, RuinResult> getAllRuins() {
        ...
        PasterDreamMod.LOGGER.debug("[RuinAPI] 📊 获取所有已注册遗迹: 共 {} 个", count);
    ...
    public static boolean hasRuin(String name) {
        ...
        PasterDreamMod.LOGGER.debug("[RuinAPI] 🔍 检查遗迹是否存在: {} → {}", name, exists);
        return exists;
    }
    ...
    public static void cacheRuin(RuinResult result) {
        if (result != null && result.name() != null) {
            ...
            PasterDreamMod.LOGGER.info("[RuinAPI] 📦 已缓存遗迹: {} | 缓存总数: {} | structureKey={} | hasSetKey={}",
                    result.name(), total, result.structureKey(), result.hasSetKey());
        } else {
            PasterDreamMod.LOGGER.warn("[RuinAPI] ⚠️ 尝试缓存 null 或无名遗迹结果");
        }
    }
