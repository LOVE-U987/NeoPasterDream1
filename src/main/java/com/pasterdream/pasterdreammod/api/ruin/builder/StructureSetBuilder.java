...
public class StructureSetBuilder {
    ...
    public StructureSetBuilder(String modId, String ruinName, String setName) {
        ...
        PasterDreamMod.LOGGER.debug("[StructureSetBuilder] 创建结构集构建器: modId={}, ruin={}, setName={}", modId, ruinName, setName);
    ...
    public StructureSetBuilder spacing(int spacing) {
        ...
        PasterDreamMod.LOGGER.debug("[StructureSetBuilder] {} → spacing={}", setName, spacing);
        return this;
    }
    ...
    public StructureSetBuilder separation(int separation) {
        ...
        PasterDreamMod.LOGGER.debug("[StructureSetBuilder] {} → separation={}", setName, separation);
        return this;
    }
    ...
    public StructureSetBuilder salt(int salt) {
        this.salt = salt;
        PasterDreamMod.LOGGER.debug("[StructureSetBuilder] {} → salt={}", setName, salt);
        return this;
    }
    ...
    public StructureSetBuilder placementType(String placementType) {
        ...
        PasterDreamMod.LOGGER.debug("[StructureSetBuilder] {} → placementType={}", setName, placementType);
        return this;
    }
    ...
    public StructureSetBuilder generateJson(boolean generate) {
        ...
        PasterDreamMod.LOGGER.debug("[StructureSetBuilder] {} → generateJson={}", setName, generate);
        return this;
    }
    ...
    public StructureSetBuilder basePath(String basePath) {
        ...
        PasterDreamMod.LOGGER.debug("[StructureSetBuilder] {} → basePath={}", setName, basePath);
        return this;
    }
...
public RuinResult build() {
        PasterDreamMod.LOGGER.info("[StructureSetBuilder] ===== 开始构建结构集: {} =====", setName);
        PasterDreamMod.LOGGER.info("[StructureSetBuilder]   配置: ruin={}, spacing={}, separation={}, salt={}, placementType={}",
        ...
        if (result == null) {
            PasterDreamMod.LOGGER.error("[StructureSetBuilder] ❌ 构建失败 [{}]: 找不到关联的结构 [{}]，请先调用 RuinBuilder.build()",
                    setName, ruinName);
        ...
        if (generateJsonFiles) {
            try {
                PasterDreamMod.LOGGER.info("[StructureSetBuilder] 生成结构集 JSON: data/{}/worldgen/structure_set/{}.json", modId, setName);
                setGenerator.saveToFile(basePath);
                PasterDreamMod.LOGGER.info("[StructureSetBuilder] ✅ 结构集 JSON 生成完成: {}", setName);
            } catch (IOException e) {
                PasterDreamMod.LOGGER.error("[StructureSetBuilder] ❌ 无法生成结构集 JSON [{}]: {}", setName, e.getMessage(), e);
                throw new RuntimeException("StructureSetBuilder: 无法生成结构集资源文件 [" + setName + "]", e);
            }
        } else {
            PasterDreamMod.LOGGER.info("[StructureSetBuilder] ⏭️ 跳过 JSON 文件生成: {} (generateJson=false)", setName);
        }

        // 4. 创建新的 RuinResult（包含结构集 Key）并缓存
        RuinResult updatedResult = result.withSetKey(setName, modId);
        PasterDreamMod.LOGGER.debug("[StructureSetBuilder] 创建 RuinResult（含结构集 Key）: ruin={}, structureKey={}, setKey={}",
        ...
        PasterDreamMod.LOGGER.info("[StructureSetBuilder] ✅ 结构集构建完成: {} | spacing={}, separation={}, salt={}",
